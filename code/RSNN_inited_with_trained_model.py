...
class RSNN(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.input_weights = self.add_weight(
            ...
            initializer=const_init(np.concatenate((np.load(os.path.join(checkpoint_prefix, 'input_weights.npy')),
                                                    np.random.normal(scale=1./np.sqrt(n_in * self.input_delay),size=(n_in,n,self.input_delay-1))),
                                                  axis=2)),
            name='input_weights_spatial')
        ...
        self.recurrent_weights = self.add_weight(
            ...
            initializer=const_init(np.load(os.path.join(checkpoint_prefix, 'recurrent_weights.npy'))),
